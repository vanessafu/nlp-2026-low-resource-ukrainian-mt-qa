#!/usr/bin/env bash
# Stage 2 UKR+mix + cs-uk MT (first 10k from version1): QA + ZNO + MR + en MT sample + cs MT + GC sample.
# Dev eval: MT (en-uk), MT_CS (cs-uk), QA, SC, GC, MR.
set -euo pipefail
...
EVAL_DIR="${EVAL_DIR:-outputs/stage2_ukr_mix_cs_dev_eval}"
LOG="${LOG:-outputs/stage2_ukr_mix_cs.log}"
EPOCHS="${EPOCHS:-1}"
BEST_CKPT_FILE="${BEST_CKPT_FILE:-outputs/stage2_best_checkpoint.json}"
...
echo "Done."
echo "  Stage2 UKR+mix+cs: $STAGE2_DIR"
