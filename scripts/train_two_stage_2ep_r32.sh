#!/usr/bin/env bash
# Two-stage LoRA (r=32, max_seq_len=3072): 2 epochs each, then full dev eval.
set -euo pipefail
...
EPOCHS=2
STAGE1_DIR="${STAGE1_DIR:-outputs/stage1_mt_gc_sc_r32_2ep_lora}"
STAGE2_DIR="${STAGE2_DIR:-outputs/stage2_qa_mr_ukr_mix_r32_2ep_lora}"
FINAL_EVAL_DIR="${FINAL_EVAL_DIR:-outputs/stage2_r32_2ep_dev_eval}"
STAGE1_BEST_FILE="${STAGE1_BEST_FILE:-outputs/stage1_r32_2ep_best_checkpoint.json}"
STAGE2_BEST_FILE="${STAGE2_BEST_FILE:-outputs/stage2_r32_2ep_best_checkpoint.json}"
LOG="${LOG:-outputs/two_stage_r32_2ep_train.log}"
...
echo "========== Stage 2: UKR+mix+cs (QA + MR + MT + GC, ${EPOCHS} epochs) =========="
